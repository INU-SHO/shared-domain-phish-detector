...
Our Brand Analyzer uses search hit counts of well-known reference
brands as a popularity anchor to estimate the popularity level of
the brand under analysis. This raises a natural question: does the
choice of **industry** for these reference brands substantially affect
detection performance?

In this analysis, we vary the industry of reference brands across
three categories and measure the resulting variation in detection
accuracy. We compare this variation against the variation caused by LLM 
non-determinism, and we conduct the experiment with two different LLMs to assess the 
model-independence of the conclusion.
...
[Li et al., 2024]: **Financial**, **Online Service**, and
**Telecommunication**.
...
We conducted statistical tests on **Accuracy** as the representative
metric. In the GPT-4.1-mini setting, Recall was 1.0 across all trials,
making Accuracy, Precision, and F1 strongly correlated; in the Qwen3-8B
setting, Recall showed only minimal variation (~0.99). Accuracy was
therefore selected as the most informative single metric for the
...
The qualitative conclusion is the same, but the gap between within- and
between-industry variability is smaller. The within-industry standard
deviation (**0.0275**) and the range of mean Accuracy across industries
(**0.0260**) are of comparable magnitude. The Kruskal-Wallis test still
does not detect a significant difference (*p* = 0.1593), but the effect
is closer to the threshold than in GPT-4.1-mini. This suggests that
the influence of reference brand selection may be relatively more
visible in a model with weaker base accuracy, although it still falls
within the range of run-to-run variation observed in our experiment.
...
3. **Sample size**: With 10 runs per industry, the statistical power
   to detect small effects is limited. The non-significant result for
   Qwen3-8B (*p* = 0.1593) should be interpreted with this in mind; a
   larger number of runs may be needed to definitively rule out small
